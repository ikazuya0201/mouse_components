...
impl SlalomConfig {
    const DEFAULT_DTHETA: AngularVelocity = AngularVelocity {
        value: 3.0 * PI,
        dimension: PhantomData,
        units: PhantomData,
    };

    const DEFAULT_DDTHETA: AngularAcceleration = AngularAcceleration {
        value: 36.0 * PI,
        dimension: PhantomData,
        units: PhantomData,
    };

    const DEFAULT_DDDTHETA: AngularJerk = AngularJerk {
        value: 1200.0 * PI,
        dimension: PhantomData,
        units: PhantomData,
    };
    ...
    pub fn parameters(&self, kind: SlalomKind, direction: SlalomDirection) -> SlalomParameters {
        use SlalomKind::*;
...
mod tests {
    ...
    proptest! {
        ...
        fn test_slalom_generator(kind: SlalomKind, dir: SlalomDirection) {
            ...
            let params = SlalomConfig::default();
            let generator = SlalomGenerator::new(
