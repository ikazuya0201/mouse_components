...
use mousecore::{
    prelude::*,
    sensors::{DistanceSensor, Encoder, Imu, Motor},
...
pub struct DistanceSensorMock<const N: usize> {
    inner: Rc<RefCell<AgentSimulatorInner>>,
    pose: Pose,
    wall_storage: Rc<WallManager<N>>,
...
impl<const N: usize> DistanceSensor for DistanceSensorMock<N> {
    ...
    fn get_distance(&mut self) -> nb::Result<Random<Length>, Self::Error> {
        ...
        let pose = Pose {
            x: state.x.x.mean + self.pose.x * sin_th + self.pose.y * cos_th,
            y: state.y.x.mean + self.pose.y * sin_th - self.pose.x * cos_th,
