    ...
    U: OutputPin,
{
    ...
    pub fn init<V: DelayMs<u32>>(&mut self, delay: &'_ mut V, new_address: u8) {
    ...
    fn read_from_registers(&mut self, address: u16) -> Result<u8, VL6180XError> {
        let (address_h, address_l) = self.split_register_address(address);
        let mut i2c = self.i2c.borrow_mut();
        let mut buffer = [0];
